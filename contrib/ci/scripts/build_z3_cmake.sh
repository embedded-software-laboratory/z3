...
: ${Z3_USE_OPENMP?"Z3_USE_OPENMP must be specified"}
...
# Use OpenMP?
if [ "X${Z3_USE_OPENMP}" = "X1" ]; then
  ADDITIONAL_Z3_OPTS+=('-DZ3_USE_OPENMP=ON')
else
  ADDITIONAL_Z3_OPTS+=('-DZ3_USE_OPENMP=OFF')
fi
...
cmake \
  ...
  -DZ3_WARNINGS_AS_ERRORS=${Z3_WARNINGS_AS_ERRORS} \
  "${ADDITIONAL_Z3_OPTS[@]}" \
  "${Z3_SRC_DIR}"
