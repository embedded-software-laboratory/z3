...
public:
    seq_factory(ast_manager &m, family_id fid, proto_model &md)
        : value_factory(m, fid), m_model(md), m(m), u(m), m_next(0), m_unique_delim("!"), m_trail(m)
    {
    ...
    void add_trail(expr *e) { m_trail.push_back(e); }

    void set_prefix(char const *p) { m_unique_delim = p; }
...
class theory_seq_empty : public theory
{
    ...
    virtual bool internalize_atom(app *, bool)
    {
        if (!m_used)
        {
            get_context().push_trail(value_trail<context, bool>(m_used));
            m_used = true;
        }
        return false;
    }
