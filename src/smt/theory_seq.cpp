...
struct display_expr
{
    ...
    std::ostream &display(std::ostream &out, sym_expr *e) const { return e->display(out); }
};
...
public:
    seq_expr_solver(ast_manager &m, smt_params &fp) : m_kernel(m, fp) {}
...
bool theory_seq::branch_variable_mb()
{
    ...
    for (unsigned i = 0; i < m_eqs.size(); ++i)
    {
        ...
        rational l1, l2;
        for (unsigned j = 0; j < len1.size(); ++j)
            l1 += len1[j];
        for (unsigned j = 0; j < len2.size(); ++j)
            l2 += len2[j];
        if (l1 != l2)
        {
...
bool theory_seq::split_lengths(dependency *dep,
                               ...
                               vector<rational> const &ll,
                               vector<rational> const &rl)
{
    context &ctx = get_context();
    ...
    expr_ref le2(m_autil.mk_le(mk_sub(mk_sub(lenXE, lenb), lenYE), m_autil.mk_int(0)), m);
...
bool theory_seq::find_branch_candidate(unsigned &start,
                                       dependency *dep,
                                       expr_ref_vector const &ls,
                                       expr_ref_vector const &rs)
{
...
bool theory_seq::propagate_length_coherence(expr *e)
{
    ...
    TRACE("seq", tout << "Unsolved " << mk_pp(e, m); if (!lower_bound(e, lo)) lo = -rational::one();
...
bool theory_seq::fixed_length(expr *e)
{
    ...
    if (is_skolem(m_tail, e) || is_skolem(m_seq_first, e) || is_skolem(m_indexof_left, e)
        || is_skolem(m_indexof_right, e) || m_fixed.contains(e))
    {
        return false;
    }
...
{
    rational r;
    return is_skolem(m_tail, e) && m_autil.is_numeral(to_app(e)->get_arg(1), r)
...
bool theory_seq::is_solved()
{
    if (!m_eqs.empty())
    {
        ...
        IF_VERBOSE(10,
                   verbose_stream() << "(seq.giveup " << m_eqs[0].ls() << " = " << m_eqs[0].rs()
                                    << " is unsolved)\n";);
        return false;
    }
    ...
    if (false && !m_nqs.empty())
    {
        ...
        IF_VERBOSE(10, display_disequation(verbose_stream() << "(seq.giveup ", m_nqs[0]);
                   verbose_stream() << " is unsolved)\n";);
        return false;
    }
    if (!m_ncs.empty())
    {
        TRACE("seq", display_nc(tout << "(seq.giveup ", m_ncs[0]); tout << " is unsolved)\n";);
        IF_VERBOSE(10, display_nc(verbose_stream() << "(seq.giveup ", m_ncs[0]);
                   verbose_stream() << " is unsolved)\n";);
        return false;
    }
...
void theory_seq::propagate_lit(dependency *dep, unsigned n, literal const *_lits, literal lit)
{
    ...
    if (!linearize(dep, eqs, lits))
        return;
    TRACE("seq", tout << "assert:"; ctx.display_detailed_literal(tout, lit); tout << " <- ";
...
void theory_seq::set_conflict(dependency *dep, literal_vector const &_lits)
{
    ...
    m_new_propagation = true;
    ctx.set_conflict(ctx.mk_justification(ext_theory_conflict_justification(
...
void theory_seq::propagate_eq(dependency *dep, enode *n1, enode *n2)
{
    ...
    if (!linearize(dep, eqs, lits))
        return;
    TRACE("seq", tout << "assert: " << mk_pp(n1->get_owner(), m) << " = " << mk_pp(n2->get_owner(), m) << " <-\n";
          display_deps(tout, dep););
...
bool theory_seq::simplify_eq(expr_ref_vector &ls, expr_ref_vector &rs, dependency *deps)
{
    ...
    TRACE("seq", if (!ls.empty() || !rs.empty()) tout << ls << " = " << rs << ";\n";
          for (unsigned i = 0; i < lhs.size();
               ++i) { tout << mk_pp(lhs[i].get(), m) << " = " << mk_pp(rhs[i].get(), m) << ";\n"; });
...
{
    if (l.size() == 1 && is_var(l[0]) && !occurs(l[0], r)
        && add_solution(l[0], mk_concat(r, m.get_sort(l[0])), deps))
    {
        return true;
    }
    if (r.size() == 1 && is_var(r[0]) && !occurs(r[0], l)
        && add_solution(r[0], mk_concat(l, m.get_sort(r[0])), deps))
    {
        return true;
    }
...
bool theory_seq::reduce_length(expr *l, expr *r, literal_vector &lits)
{
    ...
    lits.reset();
    if (get_length(l, len1, lits) && get_length(r, len2, lits) && len1 == len2)
    {
        return true;
    }
    else
...
{
    return m_util.is_seq(a) && !m_util.str.is_concat(a) && !m_util.str.is_empty(a) && !m_util.str.is_string(a)
           && !m_util.str.is_unit(a) && !m_util.str.is_itos(a) && !m.is_ite(a);
}
...
bool theory_seq::is_binary_eq(expr_ref_vector const &ls,
                              expr_ref_vector const &rs,
                              expr *&x,
                              ptr_vector<expr> &xs,
                              ptr_vector<expr> &ys,
                              expr *&y)
{
    if (ls.size() > 1 && is_var(ls[0]) && rs.size() > 1 && is_var(rs.back()))
    {
        xs.reset();
        ys.reset();
        x = ls[0];
...
bool theory_seq::reduce_length_eq(expr_ref_vector const &ls, expr_ref_vector const &rs, dependency *deps)
{
    ...
    if (ls.size() > 1 && get_length(ls.back(), len1) && get_length(rs.back(), len2) && len1 >= len2)
    {
        ...
        if (len1 == len2 && 0 < j && j < rs.size()
            && reduce_length(ls.size() - 1, rs.size() - j, false, ls, rs, deps))
        {
    ...
    if (rs.size() > 1 && get_length(rs.back(), len1) && get_length(ls.back(), len2) && len1 > len2)
    {
        ...
        if (len1 == len2 && 0 < j && j < ls.size()
            && reduce_length(ls.size() - j, rs.size() - 1, false, ls, rs, deps))
        {
...
bool theory_seq::reduce_length(
    unsigned i, unsigned j, bool front, expr_ref_vector const &ls, expr_ref_vector const &rs, dependency *deps)
{
    context &ctx = get_context();
...
bool theory_seq::solve_binary_eq(expr_ref_vector const &ls, expr_ref_vector const &rs, dependency *dep)
{
    ...
    for (unsigned offset = 0; offset < sz; ++offset)
    {
        ...
        if (!has_conflict)
        {
            TRACE("seq", tout << "offset: " << offset << " equality "; for (unsigned j = 0; j < sz; ++j) {
...
bool theory_seq::get_length(expr *e, expr_ref &len, literal_vector &lits)
{
    ...
    if (m_util.str.is_extract(e, s, i, l))
    {
        ...
        if (ctx.get_assignment(i_ge_0) == l_true && ctx.get_assignment(i_lt_len_s) == l_true
            && ctx.get_assignment(li_ge_ls) == l_true && ctx.get_assignment(l_ge_zero) == l_true)
        {
            len = l;
            lits.append(4, _lits);
            return true;
    ...
    else if (is_post(e, s, l))
    {
        ...
        if (ctx.get_assignment(l_ge_0) == l_true && ctx.get_assignment(l_le_len_s) == l_true)
        {
            len = l;
            lits.append(2, _lits);
            return true;
...
bool theory_seq::explain_eq(expr *e1, expr *e2, dependency *&dep)
{
    ...
    while (i < v1.size() || j < v2.size())
    {
        ...
        if (e1 == e2)
        {
            if (c1->m_parent && c2->m_parent && c1->m_parent->m_expr == c2->m_parent->m_expr)
            {
        ...
        else if (m_util.str.is_unit(e1, a1) && m_util.str.is_unit(e2, a2))
        {
...
bool theory_seq::internalize_atom(app *a, bool)
{
    ...
    if (m_util.str.is_contains(a, e1, e2) || m_util.str.is_prefix(a, e1, e2) || m_util.str.is_suffix(a, e1, e2))
    {
...
bool theory_seq::internalize_re(expr *e)
{
    ...
    if (m_util.re.is_star(e, e1) || m_util.re.is_plus(e, e1) || m_util.re.is_opt(e, e1)
        || m_util.re.is_loop(e, e1, lc) || m_util.re.is_loop(e, e1, lc, uc) || m_util.re.is_complement(e, e1))
    {
        return internalize_re(e1);
    }
    if (m_util.re.is_union(e, e1, e2) || m_util.re.is_intersection(e, e1, e2) || m_util.re.is_concat(e, e1, e2))
    {
        return internalize_re(e1) && internalize_re(e2);
    }
    if (m_util.re.is_full(e) || m_util.re.is_empty(e))
    {
        return true;
    }
...
literal theory_seq::is_digit(expr *ch)
{
    ...
    {
        add_axiom(~mk_eq(ch, bv.mk_numeral(rational('0' + i), bv.mk_sort(8)), false),
                  mk_eq(d2i, m_autil.mk_int(i), false));
    }
    return isd;
}
...
{
    if (m_eqs.size() == 0 && m_nqs.size() == 0 && m_rep.empty() && m_exclude.empty())
    {
        return;
    }
    out << "Theory seq\n";
...
void theory_seq::init_model(model_generator &mg)
{
    ...
    mg.register_factory((value_factory *)m_factory);
    for (ne const &n : m_nqs)
    {
...
model_value_proc *theory_seq::mk_value(enode *n, model_generator &mg)
{
    ...
    if (m.is_ite(e, e1, e2, e3) && ctx.e_internalized(e2) && ctx.e_internalized(e3)
        && (ctx.get_enode(e2)->get_root() == n->get_root() || ctx.get_enode(e3)->get_root() == n->get_root()))
    {
...
{
    if (!m_util.is_seq(n->get_owner()) && !m_util.is_re(n->get_owner()))
    {
...
expr_ref theory_seq::expand1(expr *e0, dependency *&eqs)
{
    ...
    {
        if (ctx.e_internalized(e) && ctx.e_internalized(e2)
            && ctx.get_enode(e)->get_root() == ctx.get_enode(e2)->get_root())
        {
        ...
        else if (ctx.e_internalized(e) && ctx.e_internalized(e2)
                 && ctx.get_enode(e)->get_root() == ctx.get_enode(e3)->get_root())
        {
...
void theory_seq::add_indexof_axiom(expr *i)
{
    ...
    rational r;
    VERIFY(m_util.str.is_index(i, t, s) || m_util.str.is_index(i, t, s, offset));
...
void theory_seq::add_length_axiom(expr *n)
{
    ...
    if (m_util.str.is_concat(x) || m_util.str.is_unit(x) || m_util.str.is_empty(x) || m_util.str.is_string(x))
    {
        expr_ref len(n, m);
        m_rewrite(len);
        SASSERT(n != len);
...
static bool get_arith_value(context &ctx, theory_id afid, expr *e, expr_ref &v)
{
    ...
    if (tha)
        return tha->get_value(ctx.get_enode(e), v);
    theory_i_arith *thi = get_th_arith<theory_i_arith>(ctx, afid, e);
    if (thi)
        return thi->get_value(ctx.get_enode(e), v);
...
bool theory_seq::get_num_value(expr *e, rational &val) const
{
    ...
    do
    {
        if (get_arith_value(ctx, m_autil.get_family_id(), next->get_owner(), _val) && m_autil.is_numeral(_val, val)
            && val.is_int())
        {
            return true;
        }
...
bool theory_seq::lower_bound(expr *_e, rational &lo) const
{
    ...
    if (tha && !tha->get_lower(ctx.get_enode(e), _lo))
        return false;
    if (!tha)
    {
        theory_i_arith *thi = get_th_arith<theory_i_arith>(ctx, m_autil.get_family_id(), e);
        if (!thi || !thi->get_lower(ctx.get_enode(e), _lo))
            return false;
    }
...
bool theory_seq::upper_bound(expr *_e, rational &hi) const
{
    ...
    theory_mi_arith *tha = get_th_arith<theory_mi_arith>(ctx, m_autil.get_family_id(), e);
    expr_ref _hi(m);
    if (tha && !tha->get_upper(ctx.get_enode(e), _hi))
        return false;
    if (!tha)
    {
        theory_i_arith *thi = get_th_arith<theory_i_arith>(ctx, m_autil.get_family_id(), e);
        if (!thi || !thi->get_upper(ctx.get_enode(e), _hi))
            return false;
    }
...
bool theory_seq::get_length(expr *e, rational &val) const
{
    ...
    while (!todo.empty())
    {
        ...
        else
        {
            ...
            if (ctx.e_internalized(len) && get_arith_value(ctx, m_autil.get_family_id(), len, len_val)
...
void theory_seq::add_axiom(literal l1, literal l2, literal l3, literal l4, literal l5)
{
    context &ctx = get_context();
    literal_vector lits;
    if (l1 == true_literal || l2 == true_literal || l3 == true_literal || l4 == true_literal || l5 == true_literal)
        return;
...
void theory_seq::propagate_eq(dependency *deps, literal_vector const &_lits, expr *e1, expr *e2, bool add_to_eqs)
{
    ...
    TRACE("seq", tout << "assert: " << mk_pp(e1, m) << " = " << mk_pp(e2, m) << " <- \n"; if (!lits.empty()) {
        ctx.display_literals_verbose(tout, lits);
        tout << "\n";
    });
    justification *js = ctx.mk_justification(ext_theory_eq_propagation_justification(
...
{
    if (m_util.str.is_index(n) || m_util.str.is_replace(n) || m_util.str.is_extract(n) || m_util.str.is_at(n)
        || m_util.str.is_empty(n) || m_util.str.is_string(n))
    {
        enque_axiom(n);
    }

    if (m_util.str.is_itos(n) || m_util.str.is_stoi(n))
    {
        add_int_string(n);
    }
...
bool theory_seq::is_acc_rej(
    symbol const &ar, expr *e, expr *&s, expr *&idx, expr *&re, unsigned &i, eautomaton *&aut)
{
    if (is_skolem(ar, e))
    {
        rational r;
    ...
    switch (ctx.get_assignment(e2_is_emp))
    {
        case l_true:
            TRACE("seq", tout << mk_pp(e, m) << ": " << mk_pp(e2, m) << " = empty\n";
                  ctx.display_literal_verbose(tout, e2_is_emp);
                  tout << "\n";);
            return false; // done
        case l_undef:
