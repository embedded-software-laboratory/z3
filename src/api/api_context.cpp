...
namespace api
{
object::object(context &c) : m_ref_count(0), m_context(c)
{
    this->m_id = m_context.add_object(this);
}
...
context::set_interruptable::set_interruptable(context &ctx, event_handler &i) : m_ctx(ctx)
{
...
expr *context::mk_numeral_core(rational const &n, sort *s)
{
    ...
    {
        uint64 sz;
        if (m_datalog_util.try_get_size(s, sz) && sz <= n.get_uint64())
        {
