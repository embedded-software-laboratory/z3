...
public:
    ...
    virtual tactic *translate(ast_manager &m) { return alloc(symmetry_reduce_tactic, m); }
...
public:
    ...
    {
        if ((f->is_associative() && f->is_commutative()) || m_manager.is_distinct(f))
        {
...
public:
    ac_rewriter_star(ast_manager &m) : rewriter_tpl<ac_rewriter_cfg>(m, false, m_cfg), m_cfg(m) {}
};
...
private:
    ...
    void find_candidate_permutations(expr *fml, app_map const &occs, permutations &P)
    {
        ...
        for (; it != end; ++it)
        {
            ...
            TRACE("symmetry_reduce", for (unsigned i = 0; i < it->m_value.size(); ++i) {
    ...
    struct u_pair
    {
        ...
        struct hash
        {
            unsigned operator()(u_pair const &p) const { return mk_mix(p.m_first, p.m_second, 23); }
        };
        struct eq
        {
    ...
    public:
        sort_colors(ast_manager &m, app_map &app2sort) : m_manager(m), m_app2sortid(app2sort), m_max_id(0) {}
    ...
    bool is_uninterpreted(app *t) const { return t->get_family_id() == null_family_id; }
            ...
            for (unsigned i = 0; i < sz; ++i)
            {
                ...
                if (is_app(e))
                {
                    ...
                    if (!entry->get_data().m_value)
                        entry->get_data().m_value = alloc(fun_set);
    ...
    public:
        ...
        void operator()(app *n)
        {
            ...
            for (unsigned i = 0; i < sz; ++i)
            {
                ...
                if (!entry->get_data().get_value())
                    entry->get_data().m_value = alloc(uint_set);
    ...
    bool invariant_by_permutations(expr *fml, permutation &p)
    {
        ...
        TRACE("symmetry_reduce", if (result) { tout << "Symmetric: "; } else {
            tout << "Not symmetric: ";
        } for (unsigned i = 0; i < p.size(); ++i) {
            tout << mk_pp(p[i], m()) << " ";
        } tout << "\n";);
    ...
    app *select_most_promising_term(
        expr *fml, term_set const &T, term_set &cts, term_set const &consts, app_map const &occs)
    {
        SASSERT(!T.empty());
        app *t = T[0];
    ...
    void compute_used_in(app *t, term_set &cts, term_set const &consts)
    {
        ...
        for_each_expr(mem, t);
        TRACE("symmetry_reduce", tout << "Term: " << mk_pp(t, m()) << "\n"; tout << "Support set: ";
              for (unsigned i = 0; i < consts.size(); ++i) { tout << mk_pp(consts[i], m()) << " "; } tout << "\n";
              tout << "Constants: ";
              for (unsigned i = 0; i < cts.size(); ++i) { tout << mk_pp(cts[i], m()) << " "; } tout << "\n";);
    }
