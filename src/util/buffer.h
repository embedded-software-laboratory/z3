...
public:
    ...
    buffer() : m_buffer(reinterpret_cast<T *>(m_initial_buffer)), m_pos(0), m_capacity(INITIAL_SIZE) {}

    buffer(const buffer &source)
        : m_buffer(reinterpret_cast<T *>(m_initial_buffer)), m_pos(0), m_capacity(INITIAL_SIZE)
    {
    ...
    void append(const buffer &source) { append(source.size(), source.c_ptr()); }
